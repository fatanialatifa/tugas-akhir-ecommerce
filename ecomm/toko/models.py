...
PILIHAN_KATEGORI = (
    ('MSC', 'Mesin Cuci & Pengering'),
    ('WH', 'Water Heater'),
    ('PDP', 'Perangkat Dapur'),
    ('TV', 'Televisi'),
    ('AC', 'Pendingin Ruangan'),
    ('EL', 'Elektronik Lainnya')
)
...
class ProdukItem(models.Model):
    ...
    formatted_harga= f"(harga:,)"
    formatted_harga_diskon= f"(harga_diskon:,)"
    slug = models.SlugField(unique=True)
    deskripsi = models.TextField()
    gambar = models.ImageField(upload_to='product_pics')
    label = models.CharField(choices=PILIHAN_LABEL, max_length=4)
    kategori = models.CharField(choices=PILIHAN_KATEGORI, max_length=5)
